...
import { Upload, FileAudio, Image as ImageIcon, X, Wand2, Loader2, FileText, ArrowLeft, Download, Compass, CheckCircle2, AlertTriangle, Lightbulb, MessageSquare, Microscope, Sword, Send, User, Bot, Sparkles, GraduationCap } from 'lucide-react';
...
const TEACHING_DIAGNOSIS_PROMPT = (classType: string, customDirection: string, classSize: string) => `
You are an expert Academic Director (Teaching Trainer) at Marvellous Education.
Your task is to analyze a recording/transcript of an English class and provide a professional diagnosis.

**CONTEXT:**
- Class Type: ${classType}
- Class Size: ${classSize}
- Custom Focus: ${customDirection || "Comprehensive Analysis"}

**ANALYSIS GOALS:**
1. **Instruction Checking (ICQs):** Did the teacher verify understanding?
2. **TTT vs STT:** Teacher Talking Time vs Student Talking Time. Did the student speak enough?
3. **Scaffolding:** Did the teacher support the student when they struggled?
4. **Error Correction:** Was feedback effective and encouraging?
5. **Engagement:** Was the student interested?

**OUTPUT FORMAT (Markdown):**
# 🎓 教学诊断报告 (Teaching Diagnosis)

## 1. 📊 课堂概览 (Overview)
- **总体评分 (0-100)**: [Score]
- **关键亮点**: [3 bullet points]
- **核心问题**: [3 bullet points]

## 2. 🔬 深度拆解 (Deep Dive)
### 2.1 指令与互动 (Instructions & Interaction)
> 引用老师的原话...
- **点评**: [Analysis]
- **优化建议**: [Better instruction]

### 2.2 纠错与反馈 (Feedback)
> 引用纠错片段...
- **点评**: [Analysis]

### 2.3 教学节奏 (Pacing & Scaffolding)
[Analysis]
...
export const CaseDiagnosis: React.FC<CaseDiagnosisProps> = ({ importedAudio, onClearImport }) => {
  const { user } = useAuth();
  const [selectedClassType, setSelectedClassType] = useState<string>(CLASS_TYPES[0]);
  const [selectedClassSize, setSelectedClassSize] = useState<string>(CLASS_SIZES[0]);
  ...
  const handleAnalysis = async () => {
    ...
    try {
      ...
      if (audioFile) {
        ...
        const CONCURRENCY_LIMIT = 5; // Increased to 5 to speed up processing
      ...
      if (fullTranscript) {
         ...
         response = await sendMessageToGemini({
            message: TEACHING_DIAGNOSIS_PROMPT(selectedClassType, customDirection, selectedClassSize) + `\n\n=== 录音转录内容 ===\n${fullTranscript}`,
            images: images,
  ...
  const sendFollowUp = async (text: string, isHiddenContext: boolean = false) => {
    ...
    const historyContext = followUpMessages.map(m => `${m.role === MessageRole.USER ? 'User' : 'Academic Director'}: ${m.text}`).join('\n');
    ...
    const fullPrompt = `
      ...
      [INSTRUCTION]:
      You are the Academic Director (Teaching Trainer).
      Answer the user's request specifically based on the context of the analyzed class.
      If they ask for "Example", provide a script.
      If they ask for "Theory", explain the pedagogy (ICQs, CCQs, Scaffolding).
  ...
  const handleInstructionPolish = (phase: string) => {
    const prompt = `针对【${phase}】环节，请进行“指令精修”。\n找出老师在这个环节的指令有什么问题（如复杂、歧义），并给出更清晰、符合学生水平的替代指令（ICQs/CCQs）。\n请用表格形式对比：[原指令] vs [问题] vs [精修指令]`;
    setFollowUpMessages(prev => [...prev, {
      id: Date.now().toString(),
      role: MessageRole.USER,
      text: `🔬 请对【${phase}】环节进行指令精修`
    }]);
    sendFollowUp(prompt, true);
  };

  const handleSimulate = (phase: string) => {
    const prompt = `我想针对【${phase}】环节进行模拟教学。\n请你扮演一个典型的${selectedClassType}学生（${selectedClassSize}），重现当时的场景，让我（老师）重新尝试教学。\n请直接以学生的身份开始互动。`;
    setFollowUpMessages(prev => [...prev, {
      id: Date.now().toString(),
      role: MessageRole.USER,
      text: `⚔️ 我想针对【${phase}】进行模拟教学训练`
    }]);
    sendFollowUp(prompt, true);
  };

  const handleActionPlan = () => {
    const prompt = `基于这个课堂诊断，请给出一份“下节课改进计划”。\n具体到：下节课应该重点关注什么？备课时要注意什么？`;
    setFollowUpMessages(prev => [...prev, {
      id: Date.now().toString(),
      role: MessageRole.USER,
      text: `📋 请给出下节课的改进计划`
  ...
  const handleDownload = async (type: 'image' | 'pdf') => {
    ...
    try {
      ...
      if (type === 'image') {
        ...
        link.download = `ME_Teaching_Diagnosis_${new Date().getTime()}.png`;
        ...
        pdf.save(`ME_Teaching_Diagnosis_${new Date().getTime()}.pdf`);
      }
    } catch (e) {
      alert("下载失败，请重试");
    }
    ...
    <div className="h-full flex flex-col gap-6 w-full max-w-[95%] mx-auto p-4 md:p-0 overflow-y-auto pb-20">
      ...
      <div className={`bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex-shrink-0 transition-all ${report ? 'hidden' : 'block'}`}>
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label className="block text-xs font-semibold text-slate-500 mb-2 uppercase tracking-wider">1. 选择课程类型</label>
                ...
                <label className="block text-xs font-semibold text-slate-500 mb-2 uppercase tracking-wider">2. 班级规模</label>
            ...
            <div>
                 ...
                 <input
                    ...
                    placeholder="例如：重点分析ICQ指令检查，或学生开口机会..."
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          >
            ...
            <span className="text-sm text-slate-600 font-medium">上传板书/课件截图</span>
          ...
          >
            ...
            <span className="text-sm text-slate-600 font-medium">上传课堂录音</span>
            ...
            <div className="flex flex-col items-center w-full">
               ...
               {progressPercentage > 0 && progressPercentage < 100 && (
                 <div className="w-full max-w-xs mt-2">
                   <div className="w-full bg-navy-800/30 rounded-full h-2.5 mb-1 overflow-hidden">
                     <div 
                       className="bg-gold-500 h-2.5 rounded-full transition-all duration-500 ease-out" 
        ...
        <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex justify-between items-center sticky top-0 z-20">
             <div className="flex items-center gap-4">
               ...
               <h2 className="font-bold text-slate-800 text-lg flex items-center gap-2">
                 <Wand2 className="text-navy-600" size={20}/> 教学诊断报告
               </h2>
             </div>
             <div className="flex gap-2">
          ...
          <div className="bg-slate-100 rounded-2xl p-6 border border-slate-200 shadow-inner">
             ...
             <div className="flex flex-wrap gap-3 mb-6">
                ...
                {['指令检查', '纠错反馈', '引导发问'].map(phase => (
                   <button 
                      key={`deep-${phase}`}
                      onClick={() => handleInstructionPolish(phase)}
             ...
             <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[300px] max-h-[600px]">
                ...
                <div className="p-3 bg-white border-t border-slate-200 flex gap-2">
                   <input
                      ...
                      placeholder="针对报告内容，您可以继续追问 (例如: 老师的这个指令有什么问题?)"
                      disabled={isFollowUpLoading}
