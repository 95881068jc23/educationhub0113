    ...
    <div className="h-full flex flex-col md:flex-row max-w-7xl mx-auto p-4 md:p-0 gap-6 relative overflow-hidden">
      <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ${activeQuestion ? 'md:mr-[400px]' : 'w-full'}`}>
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6 flex-shrink-0">
          <header className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                <ClipboardList className="text-blue-600" /> ME 金牌顾问 SOP (2025专家复盘版)
              </h2>
          ...
          <div className="flex w-full bg-slate-100 rounded-xl p-1 relative">
             ...
             <button onClick={() => setStage('first_half')} className={`flex-1 relative z-10 py-3 text-sm font-bold flex items-center justify-center gap-2 ${stage === 'first_half' ? 'text-blue-700' : 'text-slate-500'}`}>
               <BrainCircuit size={16} /> <span className="hidden md:inline">上半场:诊断挖需</span>
             </button>
             <button onClick={() => setStage('second_half')} className={`flex-1 relative z-10 py-3 text-sm font-bold flex items-center justify-center gap-2 ${stage === 'second_half' ? 'text-green-700' : 'text-slate-500'}`}>
               <Flag size={16} /> <span className="hidden md:inline">下半场:逻辑成交</span>
             </button>
             <button onClick={() => setStage('contract_guarantee')} className={`flex-1 relative z-10 py-3 text-sm font-bold flex items-center justify-center gap-2 ${stage === 'contract_guarantee' ? 'text-purple-700' : 'text-slate-500'}`}>
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pb-20 pr-2">
              ...
              <div key={item.id} className={`bg-white rounded-xl shadow-sm border transition-all duration-200 group relative overflow-hidden ${activeQuestion?.id === item.id ? (stage === 'contract_guarantee' ? 'border-purple-400 ring-2 ring-purple-100' : 'border-blue-400 ring-2 ring-blue-100') : 'border-slate-200 hover:shadow-md'}`}>
                 ...
                 <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center pl-5">
                    <div className="flex items-center gap-3">
                       <span className={`px-2 py-0.5 rounded-md font-bold text-[10px] uppercase tracking-wide ${stage === 'contract_guarantee' ? 'bg-purple-100 text-purple-700' : (item.isOptional ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700')}`}>
                 ...
                 <div className="p-5 grid grid-cols-1 md:grid-cols-12 gap-6 pl-5">
                    <div className="md:col-span-5">
                       ...
                       <p className="text-base font-medium text-slate-800 leading-relaxed">{item.question}</p>
                    </div>
           ...
           <div className="mt-8 flex justify-center gap-4 pb-10">
              {stage === 'first_half' ? (
                <button onClick={() => setStage('second_half')} className="bg-slate-900 text-white px-8 py-3 rounded-full font-bold flex items-center gap-2 hover:bg-slate-800 shadow-xl transition-all">
                   诊断完毕，进入逻辑成交 <ArrowRight size={18}/>
                </button>
              ) : stage === 'second_half' ? (
                <button onClick={() => setStage('contract_guarantee')} className="bg-purple-600 text-white px-8 py-3 rounded-full font-bold flex items-center gap-2 hover:bg-purple-700 shadow-xl transition-all">
                   <ShieldCheck size={18}/> 展示合同对赌保障
                </button>
              ) : (
                <button onClick={() => setStage('first_half')} className="bg-white text-slate-600 border border-slate-200 px-8 py-3 rounded-full font-bold flex items-center gap-2 hover:bg-slate-50 transition-all">
      ...
      {activeQuestion && (
        <div className="fixed inset-y-0 right-0 w-full md:w-[400px] bg-white shadow-2xl z-50 flex flex-col border-l border-slate-200 transition-all duration-300 transform translate-x-0">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
             {chatMessages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.role === MessageRole.USER ? 'justify-end' : 'justify-start'}`}>
                   <div className={`max-w-[85%] rounded-2xl p-4 text-sm shadow-sm ${msg.role === MessageRole.USER ? (stage === 'contract_guarantee' ? 'bg-purple-600 text-white rounded-br-none' : 'bg-blue-600 text-white rounded-br-none') : 'bg-white border border-slate-200 rounded-bl-none text-slate-800'}`}>
