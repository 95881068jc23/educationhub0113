  ...
  return (
    <div className="h-full flex flex-col md:flex-row max-w-7xl mx-auto p-4 md:p-0 gap-6 relative overflow-hidden">
      <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ${activeQuestion ? 'md:mr-[400px]' : 'w-full'}`}>
        <div className="bg-white rounded-2xl shadow-sm border border-navy-100 p-6 mb-6 flex-shrink-0">
          <header className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h2 className="text-xl font-bold text-navy-800 flex items-center gap-2">
                <ClipboardList className="text-navy-600" /> ME 金牌顾问 SOP (2025专家复盘版)
              </h2>
              <p className="text-sm text-navy-500 mt-1">深度挖掘需求，精准打击异议，逻辑锁定成交。</p>
            </div>
            <div className="flex bg-navy-50 p-1 rounded-lg self-start md:self-auto overflow-x-auto">
               {Object.values(ProductType).map((type) => (
                  <button key={type} onClick={() => { setSelectedProduct(type); setStage('first_half'); }}
                    className={`px-4 py-2 text-xs font-bold rounded-md transition-all whitespace-nowrap ${selectedProduct === type ? 'bg-white text-navy-600 shadow-sm' : 'text-navy-500 hover:text-navy-700'}`}>
                    {type}
                  </button>
               ))}
            </div>
          ...
          <div className="flex w-full bg-navy-50 rounded-xl p-1 relative">
             ...
             <button onClick={() => setStage('first_half')} className={`flex-1 relative z-10 py-3 text-sm font-bold flex items-center justify-center gap-2 ${stage === 'first_half' ? 'text-navy-700' : 'text-navy-500'}`}>
               <BrainCircuit size={16} /> <span className="hidden md:inline">上半场:诊断挖需</span>
             </button>
             <button onClick={() => setStage('second_half')} className={`flex-1 relative z-10 py-3 text-sm font-bold flex items-center justify-center gap-2 ${stage === 'second_half' ? 'text-gold-600' : 'text-navy-500'}`}>
               <Flag size={16} /> <span className="hidden md:inline">下半场:逻辑成交</span>
             </button>
             <button onClick={() => setStage('contract_guarantee')} className={`flex-1 relative z-10 py-3 text-sm font-bold flex items-center justify-center gap-2 ${stage === 'contract_guarantee' ? 'text-navy-900' : 'text-navy-500'}`}>
               <ShieldCheck size={16} /> <span className="hidden md:inline">核心合同保障</span>
             </button>
          </div>
        </div>

        <div className="flex-1 overflow-y-auto space-y-4 pb-20 pr-2">
           <div className="text-xs font-bold text-navy-400 uppercase tracking-wider mb-2 pl-1 flex justify-between items-center">
             ...
             {stage !== 'contract_guarantee' && (
                <div className="flex gap-4">
                  <span className="flex items-center gap-1 text-navy-700"><Star size={12} fill="currentColor"/> 核心必问</span>
           ...
           {currentItems.map((item) => (
              <div key={item.id} className={`bg-white rounded-xl shadow-sm border transition-all duration-300 group relative overflow-hidden ${activeQuestion?.id === item.id ? (stage === 'contract_guarantee' ? 'border-navy-600 ring-2 ring-navy-200' : 'border-navy-400 ring-2 ring-navy-100') : 'border-navy-200 hover:shadow-md'}`}>
                 <div className={`absolute left-0 top-0 bottom-0 w-1 ${stage === 'contract_guarantee' ? 'bg-navy-800' : (item.isOptional ? 'bg-gold-400' : 'bg-navy-600')}`}></div>
                 <div className="p-4 border-b border-navy-100 bg-navy-50/50 flex justify-between items-center pl-5">
                    <div className="flex items-center gap-3">
                       <span className={`px-2 py-0.5 rounded-md font-bold text-[10px] uppercase tracking-wide ${stage === 'contract_guarantee' ? 'bg-navy-100 text-navy-900' : (item.isOptional ? 'bg-gold-100 text-gold-700' : 'bg-navy-100 text-navy-700')}`}>
                         {stage === 'contract_guarantee' ? '权益条款' : (item.isOptional ? '选问 Deep Dive' : '必问 Core')}
                       </span>
                       <span className="font-bold text-navy-700 text-sm">{item.category}</span>
                    </div>
                    <button onClick={() => handleOpenChat(item)} className={`flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full transition-all duration-300 ${activeQuestion?.id === item.id ? (stage === 'contract_guarantee' ? 'bg-navy-800 text-white' : 'bg-navy-600 text-white') : (stage === 'contract_guarantee' ? 'bg-white text-navy-800 border border-navy-200 hover:bg-navy-50' : 'bg-white text-navy-600 border border-navy-200 hover:bg-navy-50')}`}>
                      {activeQuestion?.id === item.id ? <Bot size={14}/> : <Sparkles size={14}/>} AI 话术锦囊
                    </button>
                 </div>
                 <div className="p-5 grid grid-cols-1 md:grid-cols-12 gap-6 pl-5">
                    <div className="md:col-span-5">
                       <label className="text-[10px] font-bold text-navy-400 uppercase tracking-wider mb-2 block flex items-center gap-1">
                          {stage === 'contract_guarantee' ? <Scale size={12}/> : <Target size={12}/>} {stage === 'contract_guarantee' ? '保障内容' : '标准话术'}
                       </label>
                       <p className="text-base font-medium text-navy-800 leading-relaxed">{item.question}</p>
                    </div>
                    <div className={`md:col-span-4 p-3 rounded-lg border ${stage === 'contract_guarantee' ? 'bg-navy-50 border-navy-100' : 'bg-navy-50 border-navy-100'}`}>
                       <label className={`text-[10px] font-bold uppercase tracking-wider mb-1 block flex items-center gap-1 ${stage === 'contract_guarantee' ? 'text-navy-600' : 'text-navy-600'}`}>
                          <Lightbulb size={12}/> 核心目的
                       </label>
                       <p className="text-xs text-navy-600 leading-relaxed">{item.purpose}</p>
                    </div>
                    <div className="md:col-span-3 bg-gold-50 p-3 rounded-lg border border-gold-100">
                       <label className="text-[10px] font-bold text-gold-700 uppercase tracking-wider mb-1 block flex items-center gap-1">
                          <AlertTriangle size={12}/> 专家避坑
                       </label>
                       <p className="text-xs text-navy-600 leading-relaxed">{item.pitfall}</p>
                    </div>
                 </div>
              </div>
           ))}
           ...
           <div className="mt-8 flex justify-center gap-4 pb-10">
              {stage === 'first_half' ? (
                <button onClick={() => setStage('second_half')} className="bg-navy-800 text-white px-8 py-3 rounded-full font-bold flex items-center gap-2 hover:bg-navy-900 shadow-xl transition-all duration-300">
                   诊断完毕，进入逻辑成交 <ArrowRight size={18}/>
                </button>
              ) : stage === 'second_half' ? (
                <button onClick={() => setStage('contract_guarantee')} className="bg-navy-800 text-white px-8 py-3 rounded-full font-bold flex items-center gap-2 hover:bg-navy-900 shadow-xl transition-all duration-300">
                   <ShieldCheck size={18}/> 展示合同对赌保障
                </button>
              ) : (
                <button onClick={() => setStage('first_half')} className="bg-white text-navy-600 border border-navy-200 px-8 py-3 rounded-full font-bold flex items-center gap-2 hover:bg-navy-50 transition-all duration-300">
      ...
      {activeQuestion && (
        <div className="fixed inset-y-0 right-0 w-full md:w-[400px] bg-white shadow-2xl z-50 flex flex-col border-l border-navy-200 transition-all duration-300 transform translate-x-0">
          <div className={`p-4 flex justify-between items-center text-white ${stage === 'contract_guarantee' ? 'bg-navy-800' : 'bg-navy-600'}`}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-navy-50">
             {chatMessages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.role === MessageRole.USER ? 'justify-end' : 'justify-start'}`}>
                   <div className={`max-w-[85%] rounded-2xl p-4 text-sm shadow-sm transition-all duration-300 ${msg.role === MessageRole.USER ? (stage === 'contract_guarantee' ? 'bg-navy-800 text-white rounded-br-none' : 'bg-navy-600 text-white rounded-br-none') : 'bg-white border border-navy-200 rounded-bl-none text-navy-800'}`}>
                      <div className="prose prose-sm max-w-none prose-p:my-1 prose-strong:text-gold-500">
          ...
          <div className="p-4 bg-white border-t border-navy-200">
             <div className="relative">
                <textarea 
                  ...
                  className="w-full bg-navy-50 border border-navy-200 rounded-xl p-3 pr-12 text-sm focus:ring-2 focus:ring-navy-500 focus:outline-none h-24 resize-none shadow-inner transition-all duration-300"
                />
                <button 
                  onClick={handleSendChat} 
                  disabled={!chatInput.trim() || isChatLoading} 
                  className={`absolute right-2 bottom-2 p-2 rounded-lg text-white transition-all duration-300 active:scale-95 ${stage === 'contract_guarantee' ? 'bg-navy-800' : 'bg-navy-600'} disabled:opacity-50`}
                >
                   <Send size={18}/>
                </button>
             </div>
